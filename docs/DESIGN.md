...
| G11 | Hints (deviation) | Ribbit has no hint button. Quak has one, but it only unlocks after **1 minute of play time without a found word** (counted from puzzle start or the last found word). A hint shows translation, length, word class and pulses the starting pad of one remaining word (shortest first), then locks again for 1 minute. Hints used are shown in the results/share text. |
...
│  [Heute] [Zufällig] [Hinweis (0:31)] [Aufgeben] │
└──────────────────────────────┘
```
